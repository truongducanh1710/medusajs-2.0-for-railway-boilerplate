...
.fbc-scope {
  ...
  --border:#E5E7EB; --text-1:#111827; --text-2:#4B5563; --text-3:#9CA3AF;
...
.fbc-scope .line-clamp-1{overflow:hidden;display:-webkit-box;-webkit-line-clamp:1;-webkit-box-orient:vertical;}
.fbc-scope .line-clamp-3{overflow:hidden;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;}
.fbc-scope .hover-bg:hover{background:var(--bg-hover) !important;}
`
...
// ============================================================================
const FbContentPage = () => {
  const [tab, setTab] = useState("dangbai")
  const [prefill, setPrefill] = useState<{ videoId?: string; driveUrl?: string; sp?: string; vd?: string } | null>(null)

  useEffect(() => {
    const sp = new URLSearchParams(window.location.search)
    if (sp.get("video_id")) {
      setPrefill({ videoId: sp.get("video_id") || undefined, driveUrl: sp.get("drive_url") || undefined, sp: sp.get("sp") || undefined, vd: sp.get("vd") || undefined })
      setTab("dangbai")
      // dọn query để không prefill lại khi reload
      window.history.replaceState({}, "", "/app/fb-content")
    }
  }, [])
  ...
  return (
    <div className="fbc-scope" style={{ background: "var(--bg)", margin: -24, minHeight: "calc(100vh - 56px)" }}>
      ...
      <div style={{ padding: 20 }}>
        {tab === "dangbai" && <DangBaiTab prefill={prefill} />}
...
export const config = defineRouteConfig({ label: "Đăng Facebook" })

export default FbContentPage

...
// Một tin nhắn có thể chứa nhiều dòng giao dịch (paste hàng loạt) — dùng flag "g" để bắt hết.
const TXN_REGEX = /Visa\s+([\d.]+)\s+su dung tai\s+(.+?)\s+so tien\s+([\d,.]+)\s*(VND|USD)?\s*luc\s+(\d{2}-\d{2}-\d{4})\s+(\d{2}:\d{2}:\d{2})/gi

export function parseAdsExpenseText(text: string): ParsedAdsExpense[] {
  const results: ParsedAdsExpense[] = []
  const regex = new RegExp(TXN_REGEX)
  let match: RegExpExecArray | null

  while ((match = regex.exec(text)) !== null) {
    const [full, cardRaw, merchant, amountRaw, currency, dateStr, timeStr] = match

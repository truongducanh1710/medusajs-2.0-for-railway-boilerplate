  ...
  try {
    ...
    const rows = await cskhService.sql(`
      ...
      spend_agg AS (
        ...
        FROM camp_item_map cim
        JOIN camp_spend cs
          ON cs.campaign_name = cim.campaign_name
         AND cs.mkt_name = cim.mkt_name
        GROUP BY cim.mkt_name, cim.item_name
      )
      SELECT
        o.mkt_name,
        o.item_name,
        ...
        o.total_orders,
        o.confirmed                                                         AS don,
        CASE WHEN o.confirmed > 0
          THEN COALESCE(s.spend, 0) / o.confirmed
          ELSE 0
        END                                                                 AS chi_phi_don,

...
export default function StickyBuyBar({
  ...
  // Mirror trạng thái đặt hàng từ BundleSelector (loading khi đang add → checkout)
  useEffect(() => {
    const handler = (e: Event) => {
      const detail = (e as CustomEvent).detail
      setAdding(!!detail?.adding)
    }
    window.addEventListener("pvb-order-state", handler)
    return () => window.removeEventListener("pvb-order-state", handler)
  }, [])
  ...
  // Đặt hàng gói đang chọn — BundleSelector lắng nghe và chạy flow chuẩn
  // (add cart kèm bundle metadata + pixel + redirect /checkout)
  const handleOrder = () => {
    if (adding) return
    window.dispatchEvent(new CustomEvent("pvb-order-now"))
  }
    ...
    >
      ...
      <div className="bg-white border-t border-gray-200 shadow-[0_-4px_16px_rgba(0,0,0,0.10)]">
        <div className="flex items-center gap-3 px-4 py-3">
          {/* Vùng giá/gói — bấm để cuộn lên form đổi gói */}
          <button onClick={scrollToBuy} className="flex-1 min-w-0 text-left">
            <p className="text-[11px] text-gray-400 font-medium leading-none mb-0.5 truncate">
              {displayLabel ?? product.title}
              <span className="text-blue-600 font-bold"> · Đổi gói ▸</span>
            </p>
          ...
          <button
            onClick={handleOrder}
            disabled={adding}
            className="flex-shrink-0 h-12 px-5 rounded-xl bg-blue-600 active:bg-blue-700 text-white font-black text-sm whitespace-nowrap disabled:opacity-70"
          >
